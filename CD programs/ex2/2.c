...
int main()
{
    ...
    while (fgets(line, 100, fp))
    {
        ...
        while (str != NULL)
        {
            strcpy(preprocessor,"\t\t ");            	strcpy(keyword,"\t ");
            strcpy(function,"\t");            		strcpy(identifier,"\t");
            strcpy(operator,"\t");            		strcpy(non_operators,"\t");
            strcpy(literals,"\t");            		strcpy(temp,str);
            ...
            {
                strcpy (keyword, str);                strcat (keyword, "\t");
            }
            else if (k != 13) 
            {
                strcpy (operator, str);                strcat (operator, "\t");
            }

            else if (l != 10)
            {
                strcpy (non_operators, str);                strcat (non_operators, "\t");
            }

            else if (is_number(temp))
            {
                strcpy (literals, str);                strcat (literals, "\t");
            }

            else
            {
                strcpy (identifier, str);                strcat (identifier, "\t");
            }
